...
void gotoxy(int x, int y)
{
    ...
    p.X = x;
    p.Y = y;
    SetConsolCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), p);
}
...
void move(int* x, int* y, char* 1, char c, int len)
{
...
int main()
{
    ...
    printf("%cUse arrow key to move the snake\n", 179);
    ...
    srand(time(NULL));
    int* x, * y, px = ((rand() % 59) * 2) + 2, py = (rand() % 28) + 1, len = 1;
    char c, l;
    clock_t t;
